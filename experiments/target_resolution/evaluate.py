#!/usr/bin/env python3
"""Reproducible rule-context resolver checks for non-generation validation."""
...
from shard.application.target_resolution import (  # noqa: E402
    DEFAULT_SEMANTIC_THRESHOLD,
    resolve_template,
)
from shard.application.term_ranking import embedding_status, prepare_embeddings  # noqa: E402
from shard.inference import get_chat_llm  # noqa: E402
from shard.inference.context import inference_config  # noqa: E402
...
def _environment(name, legacy_name, default=""):
    """Read a SHARD variable while accepting its pre-rename alias."""
    return os.environ.get(name, os.environ.get(legacy_name, default))


def inference_config_from_env():
    """Build the same request-scoped config shape that the UI sends to SHARD."""
    provider = _environment("SHARD_PROVIDER", "BR2SHACL_PROVIDER", "databricks").strip().lower()
    return {
        "provider": provider,
        "databricks": {
...
def real_payload_options():
    ...
    return {
        "provider": _environment("SHARD_PROVIDER", "BR2SHACL_PROVIDER", "databricks").strip().lower(),
        "embedding_model": _environment(
            "SHARD_EMBEDDING_MODEL", "BR2SHACL_EMBEDDING_MODEL", "qwen3-embedding-0-6b"
        ).strip(),
...
def real_llm_fallback(payload):
    ...
    model_id = _environment("SHARD_LLM_MODEL", "BR2SHACL_LLM_MODEL", "gemma-3-12b").strip()
...
def main():
    ...
    parser.add_argument(
        "--semantic-threshold",
        type=float,
        default=float(_environment(
            "SHARD_SEMANTIC_THRESHOLD",
            "BR2SHACL_SEMANTIC_THRESHOLD",
            str(DEFAULT_SEMANTIC_THRESHOLD),
        )),
    ...
    parser.add_argument(
        "--llm-model",
        default=_environment("SHARD_LLM_MODEL", "BR2SHACL_LLM_MODEL", "gemma-3-12b"),
        help="Chat model for real LLM fallback. Also exported as SHARD_LLM_MODEL.",
    )
    parser.add_argument(
        "--debug-rules",
    ...
    os.environ["SHARD_LLM_MODEL"] = args.llm_model

    cases = [
        (
            "asset md",
            "asset",
            ROOT / "examples" / "asset-maintenance" / "ontology.ttl",
            ROOT / "examples" / "asset-maintenance" / "business-rules.md",
        ),
        (
            "era-rinf md",
            "era",
            ROOT / "examples" / "era-rinf" / "ontology-subset.ttl",
            ROOT / "examples" / "era-rinf" / "business-rules.md",
        ),
    ]
