...
Generate compact SHACL NodeShapes using the ontology vocabulary. Prefer sh:targetClass for class-level constraints. Use sh:path with the exact ontology property mentioned by the constraint. Add sh:minCount and sh:maxCount for cardinality constraints such as “exactly one” or “at least one”. Use sh:class for object properties whose values must be instances of a target class, and sh:datatype for datatype properties such as strings, dates, and decimals. Add clear sh:message values explaining the violated data constraint.
