...
class NamespaceDetectionTests(unittest.TestCase):
    def test_asset_namespace_uses_term_coverage(self):
        graph = graph_from_turtle(
            (ROOT_DIR / "test" / "asset_maintenance_ontology.ttl").read_text(encoding="utf-8")
        )
    ...
    def test_generic_prefixes_do_not_inject_era_aliases(self):
        ...
        legacy = ns_utils.ensure_legacy_era_aliases(prefixes, base_namespace)
        self.assertIn("@prefix era: <https://example.org/vocab#> .", legacy)
        self.assertIn("@prefix era-sh: <https://example.org/vocab/shapes/> .", legacy)
