...
def _config_fingerprint(payload):
    ...
    if config:
        try:
            from shard.inference.context import config_fingerprint
...
def _prepare_worker(key, terms, embedding_model_id, job_id, cancel_event, inference_config_payload):
    from shard.inference.context import reset_inference_config, set_inference_config

    ctx_token = set_inference_config(inference_config_payload)
    try:
        from shard.inference import get_embedding_function  # imported lazily (heavy deps)
