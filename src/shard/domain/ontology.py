"""
Shared ontology parsing helpers for SHARD workflows.

All workflows parse uploaded ontology content through this module so format
...
def ontology_base_namespace(graph: Graph) -> str:
    return namespaces.derive_base_namespace(graph)


def ontology_namespace_analysis(graph: Graph) -> Dict[str, Any]:
    return namespaces.analyze_base_namespace(graph)


def ontology_shapes_namespace(graph: Graph, base_namespace: str) -> Tuple[str, str]:
    return namespaces.derive_shapes_namespace(graph, base_namespace)


def ontology_shape_prefix(graph: Graph, shape_namespace: str) -> Tuple[str, str]:
    return namespaces.derive_shape_prefix(graph, shape_namespace)
...
def ontology_prefix_block(
    ...
    return namespaces.build_prefix_block(
        graph,
        base_namespace,
