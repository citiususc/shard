"""
Namespace analysis and editable-prefix support for SHARD.

Genericity helpers so the pipeline is not hard-wired to a domain namespace.

These helpers derive a sensible base namespace and a matching prefix block from
an arbitrary uploaded ontology, while still letting the caller override both.
The UI exposes separate ontology/shape namespace fields and an editable prefixes
panel.
"""
